			...
			$res = mysql_query($sql);
				if($res){
					header("refresh:2;url=student_course_admin.php?Sno=$Sno");
						echo "<div class='alert alert-success'>修改成功！2秒后自动返回</div>";

						}else{
					header("refresh:2;url=student_course_admin.php?Sno=$Sno");
						echo "<div class='alert alert-danger'>修改失败...2秒后自动返回</div>";

						}
						}else
						{
					echo "<script language='javascript'>alert('请重新输入成绩!');history.back();</script>";
						}
		?>
    </div> <!-- /container -->
